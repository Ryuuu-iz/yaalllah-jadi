...
<div class="px-4 py-6 sm:px-0">
    ...
    <div class="bg-white shadow rounded-lg">
        ...
        <form action="{{ route('admin.courses.store') }}" method="POST" class="px-6 py-4">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <label for="enrollment_key" class="block text-sm font-medium text-gray-700 mb-2">Enrollment Key (Optional)</label>
                <input type="text" name="enrollment_key" id="enrollment_key" value="{{ old('enrollment_key') }}"
