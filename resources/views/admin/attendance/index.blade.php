            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($groupedAttendance as $key => $groupData)
                    ...
                    <tr class="bg-gray-50 border-t-2 border-gray-300">
                        <td class="px-6 py-3 text-sm font-semibold text-gray-900" colspan="7">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    <span class="text-blue-600">📅 {{ \Carbon\Carbon::parse($date)->format('d M Y') }}</span>
                                    <span>•</span>
                                    <span>{{ $firstItem->kelas->nama_kelas }}</span>
                                    <span>•</span>
                                    <span>{{ $firstItem->mataPelajaran->nama_mapel }}</span>
                                    <span>•</span>
                                ...
                                <div class="flex gap-2">
                                    @if($course)
                                    <!-- Edit Button with proper parameters -->
                                    <a href="{{ route('admin.attendance.edit', ['id_course' => $course->id_course, 'tanggal' => $date]) }}" 
                                    ...
                                    <!-- Delete Form -->
                                    <form action="{{ route('admin.attendance.destroy') }}" method="POST" 
                                          onsubmit="return confirm('Are you sure you want to delete all attendance records for this session?')" 
                                          class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="tanggal" value="{{ $date }}">
                                        <input type="hidden" name="id_kelas" value="{{ $classId }}">
                                        <input type="hidden" name="id_mapel" value="{{ $subjectId }}">
                                    ...
                                    @else
                                    <span class="text-gray-400 text-sm">Course not found</span>
                                    @endif
                                </div>
                            </div>
                        </td>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
                            <!-- Individual actions removed, use group actions above -->
                        </td>
                    </tr>
                    @endforeach
                @empty
